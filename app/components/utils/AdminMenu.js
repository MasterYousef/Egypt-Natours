...
function AdminMenu() {
  ...
  return (
    ...
    <div className= 'bg-white fixed rounded-full cursor-pointer right-10 lg:top-11 top-3.5 hover:scale-125 ease-in-out duration-300 z-50'
      onClick={toggle}>
      {
        isOpen === true ? (<i class="fa-solid fa-xmark p-4 text-2xl"></i>):(<i class="fa-solid fa-screwdriver-wrench p-4 text-2xl"></i>)
      }
    ...
    fixed top-1/2  -translate-x-1/2 -translate-y-1/2 ease-in-out duration-500 ${isOpen && true ? 'left-1/2':'-left-1/2'}`}>
        ...
        <Link href="Admin-Tours" onClick={()=>setIsOpen(false)} className='mb-3 py-2 px-3 hov1 '>All Tours</Link>
        <Link href="Admin-Create-Tour" onClick={()=>setIsOpen(false)} className='mb-3 py-2 px-3 hov1 '>Add New Tour</Link>
